...
def parse_sites(url_):
    ...
    zak_info = parse_kontur(list_of_purch)
    win_info = parse_zakupki(list_of_purch)
    for k in zak_info:
        if win_info.get(k):
            zak_info[k]['win'] = win_info.get(k)
    return zak_info


def parser(url):
   return  parse_sites(url)
if __name__ == '__main__':
    for k,v in parser(prot_url).items():
        print(k,v)
        print('------------')